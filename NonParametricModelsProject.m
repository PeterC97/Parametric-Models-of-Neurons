...
clist = [-0.0235, 0.8, 0.5, 1]; 
klist = [0.51, 0, -0.2, 0.5]; 
for q = 1:length(clist)
    ...
    [PDMs, Npdms] = function_LET1Q2_PDMs(x, Kest, L, Nfig+1,Npdms);
    ...
    [Npdms, PDMs, ANFs, Pred, NMSE] = PDM_1(x, y, alpha, L, Nfig, ANFs_order, 3); 
    x = low_noise2;
    y = x2l; 
    ...
    alphas = linspace(0.75, 0.95, 5);  % Include 1 and exclude 0
    Ls = linspace(3,10,8);
    As = linspace(3,6,4); 
    NPs = linspace(1,6,6);
    ...
    best_values2 = Inf(1, num_best); % Initialize with Inf
    best_alphas2 = zeros(1, num_best);
    best_Ls2 = zeros(1, num_best);
    best_As2 = zeros(1, num_best); 
    best_Nps2 = zeros(1, num_best); 
        ...
        for j = 1:length(Ls)
            ...
            for l = 1: length(NPs)
                ...
                for k = 1:length(As)
                    ...
                    [max_value, max_index] = max(best_values2);
                    if sum_NMSE < max_value
                        best_values2(max_index) = sum_NMSE;
                        best_alphas2(max_index) = i;
                        best_Ls2(max_index) = j;
                        best_As2(max_index) = k; 
                        best_Nps2(max_index) = l; 
                    end 
                end 
            end 
        end
    ...
    for k = 1:num_best
        disp(['Half - Alpha ', num2str(k), ': ', num2str(alphas(best_alphas(k))), 'ANFs ', num2str(k), ': ', num2str(As(best_As(k))), 'NPDMs', num2str(k), ': ', num2str(NPs(best_Nps(k))),', L ', num2str(k), ': ', num2str(Ls(best_Ls(k)))]);
    end
    % Output the lowest 5 alphas and Ls
    for k = 1:num_best
        disp(['Alpha ', num2str(k), ': ', num2str(alphas(best_alphas2(k))), 'ANFs ', num2str(k), ': ', num2str(As(best_As2(k))), 'NPDMs', num2str(k), ': ', num2str(NPs(best_Nps2(k))),', L ', num2str(k), ': ', num2str(Ls(best_Ls2(k)))]);
    end
end 
